...
## To Do
* add uid and sample category to the do2 data 
...
############check sample_category
uid<-select(a,SITE_ID,VISIT_NO,VISIT_ID)%>%distinct()
nrow(uid) #1252
names(uid)<-tolower(names(uid))

x<-as.data.frame(table(uid$VISIT_ID))
y<-filter(x,Freq>1)


filter(uid,VISIT_ID%in%y$Var1)

filter(a,VISIT_ID==7470)%>%select(SITE_ID,VISIT_NO,VISIT_ID,SAMPLE_CATEGORY)
...
* add VISIT_ID from the 'nla2007_site_info.csv' file
    - NOTE: VISIT_ID==NA for SITE_ID=="NLA06608-3846"; change to VISIT_ID==8844
...
#THIS IS NOT WORKING  stop AND FIX
...
* profile data downloaded and UID added 
    
```{r temp, include=FALSE,eval=FALSE}

# max_temp
max_temp<- group_by(profile, VISIT_ID,VISIT_NO)%>%
  summarise(max=max(TEMP_FIELD, na.rm = TRUE))
    nrow(max_temp) #1252
...
#calculate average do for depth<=2
field_temp <- group_by(field_temp, VISIT_ID)%>%
  summarise(mean=mean(DO_FIELD, na.rm = TRUE))
...
#add parameter
field_temp$parameter<-'field_temp_2m'

#add source
field_temp$source<-'nla2007_profile.csv'

#add SAMPLE_CATEGORY
nrow(field_temp) #1245
length(unique(field_temp$VISIT_ID)) #1245

field_temp$SAMPLE_CATEGORY<-'P'

#change column names to snake case
names(field_temp)<-tolower(names(field_temp))
