...
* use a direct URL link to the raw data?
...
* This document ([nla2012_waterchem](https://github.com/willbmisled/lakes_database/blob/master/R/waterchem.Rmd)) describes  converting the water chemistry data for the 2012 National Lakes Assessment from the wide format of the raw data files to a long format for database development.  For more information see [nla2012_master.rmd](https://github.com/willbmisled/lakes_database/blob/master/R/nla2012_master.Rmd).
...
q<-formatData('nla2012_algaltoxins_08192016.csv','mcl.csv')
mcl<-q[[1]]
struc_mcl<-q[[2]]
```
...
q<-formatData('nla2012_atrazine_08192016.csv','atrazine.csv')
atrazine<-q[[1]]
struc_atrazine<-q[[2]]
```
...
```{r secchi, include=FALSE, echo=FALSE, eval=FALSE}
#read the raw data; trycatch will look for the data in two directories
secchi<-tryCatch(read.csv('data/raw_data/nla2012_secchi_08232016.csv',sep=',',na.strings = c(""," ","NA","N/A")),
            warning = function(e) read.csv('data/raw_data/nla2012_secchi_08232016.csv',sep=',',na.strings = c(""," ","NA","N/A")))
...
#rename and reorder
secchi<-select(secchi,UID,parameter,RESULT=SECCHI,UNITS,secchi_clear_to_bottom=CLEAR_TO_BOTTOM,secchi_disappears=DISAPPEARS,secchi_reappears=REAPPEARS)
...
#get the modified data structure
struc_secchi<-tryCatch(read.csv('data/workfiles/secchi.csv',sep=',',na.strings = c(""," ","NA","N/A")),warning = function(e) read.csv('../data/workfiles/secchi.csv',sep=',',na.strings = c(""," ","NA","N/A")))
kable(struc_secchi)
```

### do2

```{r do2, include=FALSE, echo=FALSE, eval=FALSE}
#read the raw data; trycatch will look for the data in two directories
do2<-tryCatch(read.csv('data/raw_data/nla2012_wide_profile_08232016.csv',sep=',',na.strings = c(""," ","NA","N/A")),warning = function(e) read.csv('data/raw_data/nla2012_wide_profile_08232016.csv',sep=',',na.strings = c(""," ","NA","N/A")))
...
#rename and reorder
do2<-select(do2,UID,parameter,RESULT=DO2_2M,UNITS)
...
```{r join, include=FALSE, echo=FALSE,eval=FALSE}
...
chem<-bind_rows(waterchem,chla,mcl,atrazine,secchi,do2)

test<-group_by(chem, parameter) %>%
